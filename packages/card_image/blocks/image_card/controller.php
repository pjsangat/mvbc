<?php
namespace Concrete\Package\CardImage\Block\ImageCard;
...
class Controller extends BlockController implements FileTrackableInterface
{
    ...
    protected $btTable = 'btContentCustomImage';
    ...
    public function getImageFeatureDetailFileObject()
    {
        ...
        $file = null;
        $fID = $db->fetchColumn('SELECT fID FROM btContentCustomImage WHERE bID = ?', [$this->bID], 0);
        if ($fID) {
            $f = File::getByID($fID);
            if (is_object($f) && $f->getFileID()) {
                $file = $f;
